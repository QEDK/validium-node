...
func (p *PostgresStorage) closeBatch(ctx context.Context, receipt ProcessingReceipt, dbTx pgx.Tx) error {
	const closeBatchSQL = `UPDATE state.batch 
		SET state_root = $1, local_exit_root = $2, acc_input_hash = $3, raw_txs_data = $4, batch_hash = $5 da_block_number = $6 da_proof = $7 da_width = $8 da_index = $9 batch_resources = $10, closing_reason = $11
		  WHERE batch_num = $12`
	...
	_, err = e.Exec(ctx, closeBatchSQL, receipt.StateRoot.String(), receipt.LocalExitRoot.String(),
		receipt.AccInputHash.String(), receipt.BatchL2Data, receipt.BatchHash, receipt.DABlockNumber, receipt.DAProof, receipt.DAWidth, receipt.DAIndex, string(batchResourcesJsonBytes), receipt.ClosingReason, receipt.BatchNumber)
